...
def main():
    ...
    for spec in args.track:
        ...
        if not args.twin_y:
            if args.scale == 'max':
                ...
                m = float(np.nanmax(y)) if np.isfinite(y).any() else 0.0
                y = y * (ref_peak / m) if m > 0 else y

        tracks.append((panel_label, axis_label, color, y, y2_ylim, y2_ticks))
    ...
    for i, (ax, (panel_label, axis_label, color, y, y2_ylim, y2_ticks)) in enumerate(zip(axes, tracks)):
