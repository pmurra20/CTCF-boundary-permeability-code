...
def export_and_plot_dynamic_curve(core_df: pd.DataFrame,
                                  ...
                                  min_measured: int = 1):
    """
    Compute dynamic fraction curve, fit exponential, write TSV and PDF.
    ...
    # Prepare fit using K>=1 points
    fit_df = curve[(curve['K'] >= 1) & np.isfinite(curve['frac_dynamic'])].copy()
    fig, ax = plt.subplots(figsize=(6, 5))

    # points
    ax.scatter(curve['K'], curve['frac_dynamic'], s=35, alpha=0.8)

    if not fit_df.empty:
        fit = _fit_exp_alpha_beta(fit_df['K'].values, fit_df['frac_dynamic'].values, weights=fit_df['den_pairs'].values)
        xs = np.linspace(0, 11, 200)
        ys = fit['alpha'] * (1.0 - np.exp(-fit['beta'] * xs))
        ax.plot(xs, ys, lw=2)
        txt = f"fit: α={fit['alpha']:.3f}, β={fit['beta']:.3f}\nPred @11: {fit['pred_11']:.3%}, R²={fit['r2']:.3f}"
        print(f"[dynamic-curve] f(K)=α(1−e^(-βK))  α={fit['alpha']:.4f}  β={fit['beta']:.4f}  pred@K=11={fit['pred_11']:.4f}  R²={fit['r2']:.4f}")
    else:
        ys = None
        txt = "no fit (insufficient K≥1 points after min_measured filter)"
        print("[dynamic-curve] no points to fit after applying min_measured; wrote TSV and scatter-only PDF")

    ax.set_xlabel('ZFs sampled (K)')
    ax.set_ylabel('Fraction dynamic (among CTCF-bound)')
    ...
    print(f"[dynamic-curve] wrote {out_prefix}.tsv and {out_prefix}.pdf")
